...
// Sets the getch() timeout depending on the snake_length.
...
void display_plot(struct world *W) {
	...
	display_timeout(W);
	erase();
	mvprintw(start_row-2,start_col,"SCORE: %u",W->snake_length);
...
static void display_timeout(struct world *W) {
	unsigned int time = MINIMUM_TIMEOUT < W->snake_length ? MINIMUM_TIMEOUT : STARTING_TIMEOUT-W->snake_length+1;
	timeout(time);
}
...
static void display_snake(struct world *W, unsigned int start_row, unsigned int start_col) {
	unsigned int i;
	struct snake *s = W->snake;
	for(i = 0; i < W->snake_length; i++, s = s->next) 
			mvaddch(start_row+s->i, start_col+s->j,SNAKE_SYMBOL);
}
