...
static void random_food(struct world *W, enum cell food) {
	unsigned int i, j;
	int tries = W->rows*W->cols-W->snake_length;
	...
	if(snake_knotted(W->snake)) {
		...
		switch(type) {
			...
			case BAD_FOOD:    // the snake ate bad food: decrease its length
				if(W->snake_length > 1) { // we don't kill a baby snake
					snake_decrease(&(W->snake),(unsigned int)(W->snake_length/2.0+0.5));
					W->snake_length /= 2;
				}
...
static enum direction select_dir(struct world *W, enum direction dir) {	
	if(W->snake_length == 1) { // Invert direction
		switch(dir) {
			case UP:    return DOWN;
			case DOWN:  return UP;	
		...
		if(head.i == neck.i) // Horizontal direction
			return head.j < neck.j ? LEFT : RIGHT;
		else                 // Vertiacal direction             
			return head.i < neck.i ? UP : DOWN;
	}
}
